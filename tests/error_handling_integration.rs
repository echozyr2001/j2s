...
fn test_very_long_file_path() {
    ...
    let input_path = temp_dir.path().join(format!("{}.json", long_name));
...
fn test_special_chars_in_path() {
    let temp_dir = TempDir::new().unwrap();
    let input_path = temp_dir.path().join("test file with spaces & symbols!.json");
...
fn test_conflicting_arguments() {
    ...
    cmd.arg(&input1) // positional
        .arg("--input").arg(&input2) // flag
        .assert()
        .success()
        .stdout(predicate::str::contains(&input2.to_string_lossy().to_string()));
}
